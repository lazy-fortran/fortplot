...
contains
    ...
    subroutine find_nice_tick_locations(data_min, data_max, target_num_ticks, &
        ...
        ! Choose nice normalized step matching matplotlib's AutoLocator step set
        ! {1, 2, 2.5, 5, 10} -- the locator linear axes use by default. (Plain
        ! MaxNLocator's wider {1, 1.5, 2, 2.5, 3, 4, 5, 6, 8, 10} is not what
        ! pyplot renders with and yields steps matplotlib never draws.)

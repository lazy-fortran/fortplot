...
module fortplot_figure_core
    ...
    use fortplot_figure_advanced_plots, only: add_imshow_impl, add_polar_impl, add_step_impl, add_stem_impl, add_pie_impl
    implicit none
...
contains
    ...
    subroutine add_imshow(self, z, xlim, ylim, cmap, alpha, vmin, vmax, origin, &
        ...
        call add_imshow_impl(fig_add_pcolormesh_wrapper, z, xlim, ylim, cmap, alpha, vmin, vmax, origin, &
                             extent, interpolation, aspect)
    contains
        subroutine fig_add_pcolormesh_wrapper(x, y, c, colormap, vmin, vmax, edgecolors, linewidths)
            real(wp), intent(in) :: x(:), y(:), c(:,:)
            character(len=*), intent(in), optional :: colormap
            real(wp), intent(in), optional :: vmin, vmax
            real(wp), intent(in), optional :: edgecolors(3)
            real(wp), intent(in), optional :: linewidths
            call self%add_pcolormesh(x, y, c, colormap, vmin, vmax, edgecolors, linewidths)
        end subroutine
    ...
    subroutine add_polar(self, theta, r, label, fmt, linestyle, marker, color)
        ...
        call add_polar_impl(fig_add_plot_wrapper, theta, r, label, fmt, linestyle, marker, color)
    contains
        subroutine fig_add_plot_wrapper(x, y, label, linestyle)
            real(wp), intent(in) :: x(:), y(:)
            character(len=*), intent(in), optional :: label, linestyle
            call self%add_plot(x, y, label, linestyle)
        end subroutine
    ...
    subroutine add_step(self, x, y, label, where, linestyle, color, linewidth)
        ...
        call add_step_impl(fig_add_plot_wrapper, x, y, label, where, linestyle, color, linewidth)
    contains
        subroutine fig_add_plot_wrapper(x, y, label, linestyle)
            real(wp), intent(in) :: x(:), y(:)
            character(len=*), intent(in), optional :: label, linestyle
    ...
    subroutine add_stem(self, x, y, label, linefmt, markerfmt, basefmt, bottom)
        ...
        call add_stem_impl(fig_add_plot_wrapper, x, y, label, linefmt, markerfmt, basefmt, bottom)
    contains
        subroutine fig_add_plot_wrapper(x, y, label, linestyle)
            real(wp), intent(in) :: x(:), y(:)
            character(len=*), intent(in), optional :: label, linestyle
    ...
    subroutine add_pie(self, values, labels, autopct, startangle, colors, explode)
        ...
        ! Convert to colors array format expected by impl
        real(wp), allocatable :: colors_array(:,:)
        
        if (present(colors)) then
            ! Note: colors string array not yet supported by impl
            call add_pie_impl(fig_add_plot_wrapper, values, labels, autopct, startangle, colors_array, explode)
        else
            call add_pie_impl(fig_add_plot_wrapper, values, labels, autopct, startangle, colors_array, explode)
        end if
    contains
        subroutine fig_add_plot_wrapper(x, y, label)
            real(wp), intent(in) :: x(:), y(:)
            character(len=*), intent(in), optional :: label
            call self%add_plot(x, y, label)
        end subroutine

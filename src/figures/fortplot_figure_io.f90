...
contains
    ...
    subroutine save_backend_with_status(backend, filename, status)
        ...
        type is (svg_context)
            call save_svg_with_status(backend, filename, status)
    ...
    subroutine save_svg_with_status(backend, filename, status)
        !! Save SVG file with status reporting
        type(svg_context), intent(inout) :: backend
        character(len=*), intent(in) :: filename
        integer, intent(out) :: status
        ...
        if (.not. file_exists .or. final_size <= 0) then
            status = ERROR_FILE_IO
            call log_error("save_svg_with_status: Failed to save SVG file")
        end if
    end subroutine save_svg_with_status

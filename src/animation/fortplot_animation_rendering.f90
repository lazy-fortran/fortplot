...
contains
    ...
    ! Removed unused internal stubs: setup_png_backend, render_to_backend,
    ! extract_png_data
    ...
    function is_valid_line_data(plot_data) result(valid)
        ...
        valid = allocated(plot_data%x) .and. allocated(plot_data%y) .and. &
                size(plot_data%x) >= 2
    ...
    subroutine set_plot_color(fig, plot_data)
        ...
        call fig%backend_color(plot_data%color(1), plot_data%color(2), &
                               plot_data%color(3))
    ...
    subroutine draw_line_segments(fig, plot_data)
        ...
        call data_to_screen_coords(fig, plot_data%x(1), plot_data%y(1), &
                                   x_prev, y_prev)

        do i = 2, size(plot_data%x)
            call data_to_screen_coords(fig, plot_data%x(i), plot_data%y(i), &
                                       x_screen, y_screen)
    ...
    subroutine data_to_screen_coords(fig, x_data, y_data, x_screen, y_screen)
        ...
        x_screen = real(fig%get_width(), wp) * (x_data - fig%get_x_min()) / &
                   (fig%get_x_max() - fig%get_x_min())
        y_screen = real(fig%get_height(), wp) * (1.0_wp - &
                   (y_data - fig%get_y_min()) / (fig%get_y_max() - fig%get_y_min()))
    ...
    subroutine draw_2d_wireframe(fig, plot_data)
        ...
        do j = 1, ny
            do i = 1, nx - 1
                call data_to_screen_coords(fig, plot_data%x(i), plot_data%y(j), &
                                           x1, y1)
                call data_to_screen_coords(fig, plot_data%x(i + 1), plot_data%y(j), &
                                           x2, y2)
        ...
        do i = 1, nx
            do j = 1, ny - 1
                call data_to_screen_coords(fig, plot_data%x(i), plot_data%y(j), &
                                           x1, y1)
                call data_to_screen_coords(fig, plot_data%x(i), plot_data%y(j + 1), &
                                           x2, y2)
                call fig%backend_line(x1, y1, x2, y2)
            end do
        end do
    ...
    subroutine draw_histogram_bars(fig, plot_data)
        ...
        do i = 1, size(plot_data%x)
            call data_to_screen_coords(fig, plot_data%x(i) - bar_width/2, &
                                       0.0_wp, x1, y_base)
            call data_to_screen_coords(fig, plot_data%x(i) + bar_width/2, &
                                       0.0_wp, x2, y_base)
            call data_to_screen_coords(fig, plot_data%x(i) - bar_width/2, &
                                       plot_data%y(i), x1, y_top)
            call data_to_screen_coords(fig, plot_data%x(i) + bar_width/2, &
                                       plot_data%y(i), x2, y_top)

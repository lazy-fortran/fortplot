...
contains
    ...
    ! --- Text width estimation using Helvetica metrics (Base14, 1000-unit em) ---
    real(wp) function estimate_pdf_text_width(text, font_size) result(width)
        ...
        !! Uses Helvetica Base14 width metrics. Handles mathtext (^, _) by parsing
        !! and summing element widths with font size ratios for superscripts.
        character(len=*), intent(in) :: text
        real(wp), intent(in), optional :: font_size
        real(wp) :: fs
    ...
    real(wp) function measure_mathtext_element_width(element, base_font_size) result(w)
        ...
        do while (i <= len_trim(element%text))
            ...
            if (char_len <= 1) then
                ...
                w = w + elem_font_size * real(helv_width_units(codepoint), wp) / 1000.0_wp
                i = i + 1
            else
                codepoint = utf8_to_codepoint(element%text, i)
                w = w + elem_font_size * real(helv_width_units(codepoint), wp) / 1000.0_wp
    ...
    integer function helv_width_units(codepoint) result(wu)
        ...
        case (88, 120)     ! X or x
            wu = 667
        case default
            ! Reasonable default for ASCII letters
            if ((codepoint >= 65 .and. codepoint <= 90)) then
                wu = 667    ! uppercase avg
            else if (codepoint >= 97 .and. codepoint <= 122) then
                wu = 444    ! lowercase avg
            else
                wu = 500
            end if
        end select

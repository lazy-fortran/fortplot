module fortplot_raster_axes
    ...
    use fortplot_tick_calculation, only: determine_decimals_from_ticks, format_tick_value_consistent
...
contains
    ...
    subroutine raster_draw_x_axis_tick_marks_only_wrapper(raster, width, height, plot_area, &
                                                 xscale, symlog_threshold, x_min, x_max, y_min, y_max)
        type(raster_image_t), intent(inout) :: raster
        integer, intent(in) :: width, height
        type(plot_area_t), intent(in) :: plot_area
        ...
        if (num_x_ticks > 0) then
            ...
            call raster_draw_x_axis_tick_marks_only(raster, width, height, plot_area, xscale, symlog_threshold, &
                x_tick_positions(1:num_x_ticks), tick_colors(:,1:num_x_ticks), x_min, x_max)
        end if
    end subroutine raster_draw_x_axis_tick_marks_only_wrapper

    subroutine raster_draw_y_axis_tick_marks_only_wrapper(raster, width, height, plot_area, &
                                                 yscale, symlog_threshold, x_min, x_max, y_min, y_max)
        type(raster_image_t), intent(inout) :: raster
        integer, intent(in) :: width, height
        type(plot_area_t), intent(in) :: plot_area
        ...
        if (num_y_ticks > 0) then
            ...
            call raster_draw_y_axis_tick_marks_only(raster, width, height, plot_area, yscale, symlog_threshold, &
                y_tick_positions(1:num_y_ticks), tick_colors(:,1:num_y_ticks), y_min, y_max)
        end if
    end subroutine raster_draw_y_axis_tick_marks_only_wrapper

    subroutine raster_draw_x_axis_tick_labels_only_wrapper(raster, width, height, plot_area, &
                                                  xscale, symlog_threshold, x_min, x_max, y_min, y_max)
        type(raster_image_t), intent(inout) :: raster
        integer, intent(in) :: width, height
        type(plot_area_t), intent(in) :: plot_area

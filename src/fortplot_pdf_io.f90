...
contains
    ...
    subroutine write_page_object(unit, ctx, pos)
        ...
        write(unit, '(A, 4(F0.1, 1X), A)') '/MediaBox [0 0 ', PDF_WIDTH, PDF_HEIGHT, ']'
        write(unit, '(A)') '/Resources <<'
        write(unit, '(A)') '  /Font <<'
        write(unit, '(A, I0, A)') '    /F', PDF_HELVETICA_OBJ, ' ', PDF_HELVETICA_OBJ, ' 0 R'
        write(unit, '(A, I0, A)') '    /F', PDF_SYMBOL_OBJ, ' ', PDF_SYMBOL_OBJ, ' 0 R'
        write(unit, '(A)') '  >>'
        write(unit, '(A)') '>>'

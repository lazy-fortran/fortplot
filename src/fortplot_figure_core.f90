module fortplot_figure_core
    ...
    use fortplot_legend
    use fortplot_png, only: png_context, raster_draw_axes_and_labels
...
contains
    ...
    subroutine render_figure(self)
        ...
        type is (png_context)
            if (allocated(self%ylabel)) then
                call backend%render_ylabel(self%ylabel)
            end if
        type is (pdf_context)
    ...
    subroutine render_figure_axes(self)
        ...
        select type (backend => self%backend)
        type is (png_context)
            call raster_draw_axes_and_labels(backend, self%xscale, self%yscale, self%symlog_threshold, &

program test_parametric_surface
    ...
    use fortplot, only: add_parametric_surface, figure, savefig, title
    use fortplot_system_runtime, only: create_directory_runtime
    use fortplot_validation, only: validate_file_size, validate_png_format, &
                                   validation_result_t
    implicit none

    integer, parameter :: meridian_count = 9, azimuth_count = 17
    ...
    do i = 1, meridian_count
        theta(i) = pi*real(i - 1, wp)/real(meridian_count - 1, wp)
    end do
    ...
    call add_parametric_surface(x, y, z, color='black', linewidth=0.8_wp, &
                                label='parametric torus')
    call title('Parametric 3D surface wireframe')

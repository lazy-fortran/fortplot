program test_bitmap_to_png_buffer
    ...
    call render_text_to_bitmap(bitmap, width, height, padding, padding + &
                               text_height, test_text)
    ...
    call test_buffer_roundtrip(rotated_bitmap, rotated_png_buffer, height, &
                               width, "rotated")

    ! Write actual PNG files using the real PNG writer
    call write_png_file("build/test/output/test_bitmap_original.png", width, height, &
                        png_buffer)
    call validate_png_file("build/test/output/test_bitmap_original.png", &
                           "Bitmap PNG test - original")
    call write_png_file("build/test/output/test_bitmap_rotated.png", height, width, &
                        rotated_png_buffer)
    call validate_png_file("build/test/output/test_bitmap_rotated.png", &
                           "Bitmap PNG test - rotated")

    print *, "SUCCESS: PNG buffer conversion tests passed"
    print *, "  bitmap_to_png_buffer format validated"
    print *, "  Created build/test/output/test_bitmap_original.png"
    print *, "  Created build/test/output/test_bitmap_rotated.png"
...
contains
    ...
    subroutine validate_png_buffer(buffer, width, height, label)
        ...
        if (actual_size /= expected_size) then
            print *, "ERROR: ", label, " buffer size mismatch - Expected:", &
        ...
        do j = 1, height
            ...
            ! PNG filter byte should be 0 for no filter.
            ! initialize_white_background sets this filter value.
        ...
        do j = 1, height
            do i = 1, width
                ...
                if (buffer(idx) /= -1_1 .or. &
                    buffer(idx + 1) /= -1_1 .or. &
                    buffer(idx + 2) /= -1_1) then
    ...
    subroutine test_buffer_roundtrip(original_bitmap, png_buffer, width, height, label)
        ...
        if (.not. pixels_match) then
            print *, "ERROR: ", label, " roundtrip failed - ", mismatched_pixels, &
                " pixels don't match"
            stop 1
        end if

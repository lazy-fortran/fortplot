program test_bitmap_rotation_both
    ...
    integer(1), allocatable :: cw_bitmap(:, :, :)
    integer(1), allocatable :: ccw_bitmap(:, :, :)
    integer(1), allocatable :: full_rotation_bitmap(:, :, :)
    integer(1), allocatable :: png_buffer(:), cw_png_buffer(:), ccw_png_buffer(:), &
                               full_png_buffer(:)
...
contains
    ...
    subroutine validate_rotation_properties(original, cw_rotated, ccw_rotated, &
                                            rotated_180, orig_width, orig_height)
        !! Validate rotation properties for both directions
        integer(1), intent(in) :: original(:, :, :), cw_rotated(:, :, :), &
                                  ccw_rotated(:, :, :), rotated_180(:, :, :)
        ...
        if (orig_pixels /= cw_pixels .or. orig_pixels /= ccw_pixels .or. orig_pixels &
            /= rot180_pixels) then
        ...
        if (size(cw_rotated, 1) /= orig_height .or. size(cw_rotated, 2) /= &
            orig_width) then
            print *, "ERROR: CW rotation dimensions incorrect"
            stop 1
        end if

        if (size(ccw_rotated, 1) /= orig_height .or. size(ccw_rotated, 2) /= &
            orig_width) then
        ...
        if (orig_width >= 3 .and. orig_height >= 3) then
            ...
            if (.not. pixels_equal(original(1, 1, :), cw_rotated(1, &
                                                                 orig_width, :))) then
            ...
            if (.not. pixels_equal(original(1, 1, :), ccw_rotated(orig_height, &
                                                                  1, :))) then
    ...
    function count_non_white_pixels(bitmap, width, height) result(count)
        ...
        do j = 1, height
            do i = 1, width
                if (bitmap(i, j, 1) /= -1_1 .or. bitmap(i, j, 2) /= -1_1 .or. &
                    bitmap(i, j, 3) /= -1_1) then
                    count = count + 1
                end if
            end do
        end do
    ...
    function pixels_equal(pixel1, pixel2) result(equal)
        ...
        equal = (pixel1(1) == pixel2(1) .and. pixel1(2) == pixel2(2) .and. pixel1(3) &
                 == pixel2(3))

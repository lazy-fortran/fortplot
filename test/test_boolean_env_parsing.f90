program test_boolean_env_parsing
    ...
    call ensure(parse_boolean_env('T'), 'T => true')
    call ensure(parse_boolean_env('t'), 't => true')
    call ensure(parse_boolean_env('Y'), 'Y => true')
    call ensure(parse_boolean_env('y'), 'y => true')
    ...
    call ensure(.not. parse_boolean_env('F'), 'F => false')
    call ensure(.not. parse_boolean_env('f'), 'f => false')
    call ensure(.not. parse_boolean_env('N'), 'N => false')
    call ensure(.not. parse_boolean_env('n'), 'n => false')

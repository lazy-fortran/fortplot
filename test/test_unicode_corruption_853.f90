program test_unicode_corruption_853
    !! Test for Issue #853: ASCII backend converts Unicode to readable ASCII.
    !!
    !! This test exercises Unicode-to-ASCII conversion and ASCII backend output.
    !!
    ...
    integer, parameter, dimension(4) :: greek_lower_codes = [945, 946, 947, 948]
...
contains
    ...
    subroutine test_unicode_to_ascii_conversion()
        ...
        if (trim(ascii_output) /= "alpha") then
            print *, "ERROR: Greek alpha should convert to 'alpha', got '", &
                trim(ascii_output), "'"
            stop 1
        end if
        ...
        if (trim(ascii_output) /= "pi") then
            print *, "ERROR: Greek pi should convert to 'pi', got '", &
                trim(ascii_output), "'"
            stop 1
        end if
        ...
        if (trim(ascii_output) /= "Omega") then
            print *, "ERROR: Greek Omega should convert to 'Omega', got '", &
                trim(ascii_output), "'"
            stop 1
        end if
        ...
        if (trim(ascii_output) /= "U+20AC") then
            print *, "ERROR: Euro symbol should convert to 'U+20AC', got '", &
                trim(ascii_output), "'"
            stop 1
        end if
    ...
    subroutine test_mathematical_symbols_conversion()
        ...
        if (trim(ascii_output) /= "U+2202") then
            print *, "ERROR: Partial derivative should get placeholder, got '", &
                trim(ascii_output), "'"
            stop 1
        end if
        ...
        if (trim(ascii_output) /= "U+2207") then
            print *, "ERROR: Nabla should get placeholder, got '", &
                trim(ascii_output), "'"
            stop 1
        end if
        ...
        if (trim(ascii_output) /= "U+00B1") then
            print *, "ERROR: Plus-minus should get placeholder, got '", &
                trim(ascii_output), "'"
            stop 1
        end if
    ...
    subroutine test_ascii_backend_unicode_handling()
        ...
        character(len=256) :: test_filename = "test/output/test_unicode_853_bug.txt"
        ...
        if (index(file_content, "Mathmb") > 0) then
            print *, "ERROR: Found Mathmb corruption pattern in ASCII output"
            stop 1
        end if

        if (index(file_content, "alpha") == 0 .or. index(file_content, "mu") == 0) then
            print *, "ERROR: Missing expected Unicode-to-ASCII conversions in output"
            stop 1
        end if

        print *, "PASS: ASCII backend Unicode output contains expected conversions"

program debug_16x16_q95
    ...
    print *, "Creating 16x16 gradient with quality 95 (no subsampling)..."
    
    ! Write with STB
    filename = "stb_16x16_q95.jpg" // c_null_char
    result = stbi_write_jpg(filename, width, height, 3, c_loc(image_data), 95)
    if (result == 0) error stop "STB write failed"
    
    ! Write with our implementation
    call write_jpeg_file("our_16x16_q95.jpg", width, height, image_data, 95)

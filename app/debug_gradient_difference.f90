program debug_gradient_difference
    implicit none
    
    integer(1), allocatable :: stb_data(:), our_data(:)
    integer :: stb_size, our_size, i, first_diff
    character(len=*), parameter :: stb_file = "debug_stb_gradient.jpg"
    character(len=*), parameter :: our_file = "debug_our_gradient.jpg"
    
    print *, "Analyzing gradient pattern differences"
    print *, "====================================="
    ...
    if (first_diff > 0) then
        ...
        if (stb_size /= our_size) then
            ...
            print *, "But sizes differ - examining the trailing byte(s)"
            
            if (stb_size > our_size) then
                print *, "STB has extra byte(s):"
                do i = our_size + 1, stb_size
                    print *, "  STB[", i, "] = 0x" // byte_to_hex(stb_data(i))
                end do
            else
                print *, "Our implementation has extra byte(s):"
                do i = stb_size + 1, our_size
                    print *, "  Our[", i, "] = 0x" // byte_to_hex(our_data(i))
                end do
            end if
        else
...
end program debug_gradient_difference